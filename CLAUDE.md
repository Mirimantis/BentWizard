...
- `freecad/bentwizard/` — the workbench package (FreeCAD 1.x addon-manager layout; `package.xml` at root)
  ...
  - `assemble.py` — the two-level structure assembly (container rules, `assimilate_joint`, Assemble Timbers). **Superseded by the flat frame** (2026-09-19, not built yet): one assembly per frame, bents and bays as Std Groups, timbers seated by expression through a spanning tree of placing joints (a `Seat_J-…` VarSet under each handle) instead of Fixed joints and the solver — see the roadmap's *Flat frame* entry before changing it
