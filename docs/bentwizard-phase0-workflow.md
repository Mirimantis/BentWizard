...
- **Joint features within bodies**: **descriptive-first** (reworked July 2026) — `<Descriptive>[.<TypeTag>].<Abbrev>.<serial>`, e.g. `Mortise.HMT.001`, `TailSlope.Skt.WHD.001`, `Mate.Lcs.WHD.001`. The name you scan for comes first; the retired `<TimberLabel>_<Feature>_<JointLabel>` form buried it between two long tokens (`T.AnchorBeam.001_TailSlopeSketch_J-WedgedHalfDovetail-000`, 57 characters). **Inside a TEMPLATE the serial is the template joint VarSet's own — `.HMT.000`, not `.001`.** The `001` in these examples is an applied instance; Apply-Joint rewrites the whole suffix when it clones, and the strict rule compares against the template's own. New Joint Template prints the exact string for the file it creates.
