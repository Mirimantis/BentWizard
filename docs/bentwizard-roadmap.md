...
- New Timber dialog: a Role picker (existing role bases in the document) feeding the auto-serial, and structured position pickers (Bent/Bay/Level) composing Position_Tag once bents exist (ties to the Phase 2 Role property); custom labels stay allowed, nudged by the advisory naming rule.
...
- Feature label scheme within bodies: resolved — body-qualified `<TimberLabel>_<Feature>_<JointLabel>` (workflow doc §3), written by Apply Joint.
