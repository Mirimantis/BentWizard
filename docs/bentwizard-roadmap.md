...
- Expression entry in the Apply Joint parameter fields (the core already accepts an expression string as a value; the dialog needs the affordance — e.g. an f(x) toggle per field). (Also answers the setback-naming concern in docs/User Notes.txt: `_Face1`/`_Face2` qualifiers are the stable square-rule references, but which physical direction they mean per timber needs to be visible.)
