...
- **Preview Mated Joint UI**: the run-again-to-clear toggle works but is unintuitive. Wants a clearer affordance (a visible preview state / dedicated clear). Live parameter tracking is done (the ghost attaches to the landing frame, following station/section/tenon-shape edits); the one gap is `Tenon_Length`, which relocates the mate frame within the secondary and so shifts the ghost until refreshed — a future refinement could drive that offset too.
