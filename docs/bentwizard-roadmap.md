...
- Visual indication of the reference faces (Face 1/Face 2) on a timber in the 3D view. (Also answers the setback-naming concern in docs/User Notes.txt: `_Face1`/`_Face2` qualifiers are the stable square-rule references, but which physical direction they mean per timber needs to be visible.)
